...
func runMigrations(databaseURL string) error {
	db, err := datasource.NewDatabase(datasource.DatabaseConfig{DatabaseURL: databaseURL})
	if err != nil {
		return err
	}
	db.Close()
...
func parseConfig() seedConfig {
	...
	flag.IntVar(&config.users, "users", envInt(seedUsersEnv, defaultUsers), "Number of users to seed")
	flag.IntVar(&config.games, "games", envInt(seedGamesEnv, defaultGames), "Number of games to seed")
	flag.Parse()
...
func (s *gameCopySource) Values() ([]any, error) {
	return s.row, nil
}

func (s *gameCopySource) Err() error {
	return s.err
}
...
func userLogin(index int) string {
	return fmt.Sprintf("%04d", index)
}

func userUUID(index int) string {
	return fmt.Sprintf("00000000-0000-4000-8000-%012d", index)
}

func gameUUID(index int) string {
