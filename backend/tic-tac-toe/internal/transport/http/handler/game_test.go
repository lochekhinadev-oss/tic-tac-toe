...
func (l *logicStub) CreateGame(uuid string, creatorUUID string, mode domain.GameMode) (domain.Game, error) {
...
func (l *logicStub) JoinGame(game domain.Game, userUUID string) (domain.Game, error) {
	if game.Mode != domain.GameModePlayer || game.State != domain.GameStateWaitingPlayers || game.PlayerXUUID == userUUID {
...
func (l *logicStub) ApplyMove(previous domain.Game, current domain.Game, userUUID string) (domain.Game, error) {
...
func (s *storageStub) GetGame(_ context.Context, uuid string) (domain.Game, error) {
...
func (s *storageStub) ListCompletedGamesByUserUUID(context.Context, string) ([]domain.Game, error) {
	if s.history != nil {
...
func (s *storageStub) JoinGame(context.Context, string, string) (domain.Game, error) {
	if s.getErr != nil {
...
func authenticatedRequest(method string, target string, body io.Reader) *http.Request {
	...
	return req.WithContext(middleware.WithUserUUID(req.Context(), "user-1"))
}
...
func withRouteUUID(next func(http.ResponseWriter, *http.Request, string)) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		next(w, r, chi.URLParam(r, "uuid"))
	}
}
